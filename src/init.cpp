...
// Copyright (c) 2018 The WealthSalvage developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/WealthSalvage-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("WealthSalvage-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "WealthSalvage.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "WealthSalvaged.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the WLTHS and zWLTHS money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in WLTHS/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in WLTHS/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, WealthSalvage, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in WLTHS/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all WealthSalvage specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzWealthSalvage=<n>", strprintf(_("Enable automatic wallet backups triggered after each zWealthSalvage minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeWealthSalvageamount=<n>     " + strprintf(_("Keep N WLTHS anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The Awesome WealthSalvage Core Developers"), 2019)) + "\n" +
           "\n" +
...
{
    RenameThread("WealthSalvage-loadblk");
...
/** Sanity checks
 *  Ensure that WealthSalvage is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. WealthSalvage Core is shutting down."));
...
#endif
    // Make sure only a single WealthSalvage process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. WealthSalvage Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("WealthSalvage version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // WealthSalvage: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZWLTHSMinted();
                        RecalculateZWLTHSSpent();
                    }
                    RecalculateWLTHSSupply(1);
                }
                ...
                // WealthSalvage: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of WealthSalvage Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart WealthSalvage Core to complete") << "\n";
        ...
        if (fFirstRun) {
            ...
            pwalletMain->SetBestChain(chainActive.GetLowlthsr());
        }
        ...
        if (GetBoolArg("-rescan", false))
            ...
            CBlockLowlthsr lowlthsr;
            if (walletdb.ReadBestBlock(lowlthsr))
                pindexRescan = FindForkInGlobalIndex(chainActive, lowlthsr);
            else
                pindexRescan = chainActive.Genesis();
        }
        ...
        bool fEnableZWealthSalvageBackups = GetBoolArg("-backupzWealthSalvage", true);
        pwalletMain->setZWealthSalvageAutoBackups(fEnableZWealthSalvageBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeWealthSalvageAmount = 2;
...
//    }
//
//    nAnonymizeWealthSalvageAmount = GetArg("-anonymizeWealthSalvageamount", 0);
//    if (nAnonymizeWealthSalvageAmount > 999999) nAnonymizeWealthSalvageAmount = 999999;
//    if (nAnonymizeWealthSalvageAmount < 2) nAnonymizeWealthSalvageAmount = 2;
    ...
    LogPrintf("Anonymize WealthSalvage Amount %d\n", nAnonymizeWealthSalvageAmount);
    ...
    /* Denominations
       ...
       For example:
       1WLTHS+1000 == (.1WLTHS+100)*10
       10WLTHS+10000 == (1WLTHS+1000)*10
    */
