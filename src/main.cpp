...
#if defined(NDEBUG)
#error "WealthSalvage cannot be compiled without assertions."
#endif
...
// XX42    boost::signals2::signal<void(const uint256&)> EraseTransaction;
    ...
    boost::signals2::signal<void(const CBlockLowlthsr&)> SetBestChain;
...
CBlockIndex* FindForkInGlobalIndex(const CChain& chain, const CBlockLowlthsr& lowlthsr)
{
    // Find the first block the caller has in the main chain
    BOOST_FOREACH (const uint256& hash, lowlthsr.vHave) {
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zWealthSalvage spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zWealthSalvage spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zWealthSalvage spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
	...
	if(nHeight == 1 ){
        return  2491000 * COIN;  // premine
        }

	if(nHeight <= 300) {
	return  120 * COIN;
	} else if (enabled_nodes >= 0 && enabled_nodes <= 29) {
		nSubsidy =  2 * COIN;
	} else if (enabled_nodes >= 30 && enabled_nodes <= 49) {
		nSubsidy = 10 * COIN;
	} else if (enabled_nodes >= 50 && enabled_nodes <= 99) {
		nSubsidy =  15 * COIN;
	} else if (enabled_nodes >= 100 && enabled_nodes <= 199) {
		nSubsidy = 28 * COIN;
	} else if (enabled_nodes >= 200 && enabled_nodes <= 399) {
                nSubsidy = 35 * COIN;
	} else if (enabled_nodes >= 400) {
                nSubsidy = 20 * COIN;
	}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from WealthSalvage
...
{
    RenameThread("WealthSalvage-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZWLTHSMinted()
{
...
void RecalculateZWLTHSSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zWLTHS supply
...
bool RecalculateWLTHSSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zWealthSalvage with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zWealthSalvageSpent: %s\n",
    ...
    try {
        if ((mode == FLUSH_STATE_ALWAYS) ||
            ...
            if (mode != FLUSH_STATE_IF_NEEDED) {
                g_signals.SetBestChain(chainActive.GetLowlthsr());
            }
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert WLTHS to zWLTHS
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zWealthSalvage spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zWealthSalvage serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zWealthSalvage mints and %d zWealthSalvage spends\n", __func__, nMints, nSpends);
    ...
    if (pblock->GetHash() != Params().HashGenesisBlock() && pfrom != NULL) {
        ...
        if (mi == mapBlockIndex.end()) {
            pfrom->PushMessage("getblocks", chainActive.GetLowlthsr(), uint256(0));
            return false;
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // WealthSalvage: We use certain sporks during IBD, so check to see if they are
        ...
        uint256 hashStop;
        vRecv >> lowlthsr >> hashStop;

        LOCK(cs_main);

        // Find the last block the caller has in the main chain
        CBlockIndex* pindex = FindForkInGlobalIndex(chainActive, lowlthsr);
        ...
        CBlockIndex* pindex = NULL;
        if (lowlthsr.IsNull()) {
            // If lowlthsr is null, return the hashStop block
            ...
            // Find the last block the caller has in the main chain
            pindex = FindForkInGlobalIndex(chainActive, lowlthsr);
            if (pindex)
                pindex = chainActive.Next(pindex);
        }
    ...
    else if (strCommand == "headers" && Params().HeadersFirstSyncingActive() && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLowlthsr(pindexLast), uint256(0));
        }
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (!mapBlockIndex.count(block.hashPrevBlock)) {
            if (find(pfrom->vBlockRequested.begin(), pfrom->vBlockRequested.end(), hashBlock) != pfrom->vBlockRequested.end()) {
                ...
                pfrom->PushMessage("getblocks", chainActive.GetLowlthsr(), block.hashPrevBlock);
                pfrom->vBlockRequested.push_back(block.hashPrevBlock);
            } else {
                //ask to sync to this block
                pfrom->PushMessage("getblocks", chainActive.GetLowlthsr(), hashBlock);
...
{
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && fFetch /*&& !fImporting*/ && !fReindex) {
            ...
            if (nSyncStarted == 0 || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) { // NOTE: was "close to today" and 24h in Bitcoin
                ...
                //pto->PushMessage("getheaders", chainActive.GetLowlthsr(pindexStart), uint256(0));
                pto->PushMessage("getblocks", chainActive.GetLowlthsr(chainActive.Tip()), uint256(0));
            }
        }
