...
#include "ui_zWealthSalvagecontroldialog.h"
...
std::list<std::string> ZWealthSalvageControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZWealthSalvageControlDialog::listMints;

ZWealthSalvageControlDialog::ZWealthSalvageControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZWealthSalvageControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZWealthSalvageControlDialog::~ZWealthSalvageControlDialog()
{
    delete ui;
}

void ZWealthSalvageControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWealthSalvageControlDialog::updateList()
{
...
void ZWealthSalvageControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWealthSalvageControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZWealthSalvage_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZWealthSalvageControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZWealthSalvageControlDialog::GetSelectedMints()
{
...
void ZWealthSalvageControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
