...
collat_required = 1999.99 * COIN;
} else if (txnheight <= GetSporkValue(SPORK_36_LAST_3000_COLLAT_BLOCK)) {
collat_required = 2999.99 * COIN;
} else if (txnheight <= GetSporkValue(SPORK_19_LAST_4000_COLLAT_BLOCK)){
collat_required = 3999.99 * COIN;
} else if (txnheight <= GetSporkValue(SPORK_20_LAST_5000_COLLAT_BLOCK)){
collat_required = 4999.99 * COIN;
} else if (txnheight <= GetSporkValue(SPORK_21_LAST_6000_COLLAT_BLOCK)){
collat_required = 5999.99 * COIN;
} else if (txnheight <= GetSporkValue(SPORK_22_LAST_7000_COLLAT_BLOCK)){
collat_required = 6999.99 * COIN;
} else {
collat_required = 7099.99 * COIN;
}
} else {
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 50000 WLTHS tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 50000 WLTHS tx -> 1 confirmation
