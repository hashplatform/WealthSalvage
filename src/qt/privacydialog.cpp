...
#include "zWealthSalvagecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zWLTHS ought to be enough for anybody." - Bill Gates, 2017
    ui->zWLTHSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzWLTHSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzWLTHS->setEnabled(false);
        ui->pushButtonMintzWLTHS->setToolTip(tr("zWLTHS is currently disabled due to maintenance."));

        ui->pushButtonSpendzWLTHS->setEnabled(false);
        ui->pushButtonSpendzWLTHS->setToolTip(tr("zWLTHS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zWLTHSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzWLTHS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zWLTHS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zWLTHS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zWLTHS in ") +
...
void PrivacyDialog::on_pushButtonSpendzWLTHS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zWLTHS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zWLTHS
        sendzWLTHS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zWLTHS
    sendzWLTHS();
}

void PrivacyDialog::on_pushButtonZWealthSalvageControl_clicked()
{
    ZWealthSalvageControlDialog* zWealthSalvageControl = new ZWealthSalvageControlDialog(this);
    zWealthSalvageControl->setModel(walletModel);
    zWealthSalvageControl->exec();
}

void PrivacyDialog::setZWealthSalvageControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzWealthSalvageSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzWLTHS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid WealthSalvage Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zWLTHSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zWLTHS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " WLTHS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zWLTHS</b>";
    ...
    // use mints from zWealthSalvage selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZWealthSalvageControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZWealthSalvageControlDialog::GetSelectedMints();
    }
    ...
    // Clear zWealthSalvage selector in case it was used
    ZWealthSalvageControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zWealthSalvage Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " WealthSalvage, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zWealthSalvage Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zWLTHS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zWLTHS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zWLTHS "));
    ui->labelzWLTHSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzWLTHSSyncStatus->setVisible(fShow);
}
